...
class DialogManager:
    def __init__(self, window, backgroundImg, margin, text, font, Location=(-1, -1)):
        self._window = window
        ...
        self.text = self.wrap_text(self._window.get_width(), text)  # Stocker le texte
    ...
    def set_text(self):
        ...
        if self._location == (-1, -1):
            self._img_rect = self._backgroundImg.get_rect(center=(self._window.get_width() // 2, self._window.get_height()))
            self._img_rect.bottom = self._window.get_height()  # Ajuste pour que l'image soit en bas
        else:
